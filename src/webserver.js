...
function createWebServer({ onPhoto, onDiorama, onModel, onPortrait, onMaison }) {
  const app = express();
  const upload = multer({
  ...
  app.get('/upload/:username', (req, res) => {
    ...
    const mode = ['diorama', 'statue', 'portrait', 'maison'].includes(req.query.mode) ? req.query.mode : '';
    const accept = mode
      ...
      try {
        ...
        if (MODEL_EXTS.has(ext)) {
          ...
          message = m === 'diorama' ? await onDiorama(req.body.username, req.file.buffer, req.file.mimetype)
            ...
            : m === 'maison' ? await onMaison(req.body.username, req.file.buffer, req.file.mimetype)
